# Version 1.9 - Implementing spaCy for Named Entity Recognition (NER) for Location Extraction
...
def load_spacy_model():
    try:
        # This assumes 'en_core_web_sm' is available.
        # If not, users might need to run: python -m spacy download en_core_web_sm
        return spacy.load("en_core_web_sm")
    except Exception as e:
        st.error(f"❌ Error loading spaCy model: {e}. Please ensure 'en_core_web_sm' is downloaded (run: python -m spacy download en_core_web_sm).")
        return None
...
def extract_location(text):
    """
    Extracts location details (cities, countries, states) using spaCy's Named Entity Recognition (NER).
    Identifies 'GPE' (Geo-Political Entity) labels.
    """
    if nlp is None:
        return "NER Model Not Loaded" # Fallback if model failed to load

    doc = nlp(text)
    locations = set()
    for ent in doc.ents:
        if ent.label_ == "GPE": # Geo-Political Entity (cities, countries, states)
            # Clean and add to set to avoid duplicates
            # Remove punctuation and extra spaces, then title case
            cleaned_loc = re.sub(r'[^\w\s]', '', ent.text).strip()
            if cleaned_loc: # Ensure it's not empty after cleaning
                locations.add(cleaned_loc.title())
...
def resume_screener_page():
    ...
    if jd_text and resume_files:
        ...
        jd_words_for_cloud_set, _ = extract_relevant_keywords(jd_text, MASTER_SKILLS) # Get raw skills for cloud
        ...
        for i, file in enumerate(resume_files):
            ...
            location = extract_location(text) # Now uses spaCy
        ...
        if not df.empty:
            ...
            jd_raw_skills_set, jd_categorized_skills_for_top = extract_relevant_keywords(jd_text, MASTER_SKILLS)
            resume_raw_skills_set_for_top, _ = extract_relevant_keywords(top_candidate['Resume Raw Text'], MASTER_SKILLS)
        ...
        jd_raw_skills_set, _ = extract_relevant_keywords(jd_text, MASTER_SKILLS) # Re-extract JD skills here for the filter
