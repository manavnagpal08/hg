...
def resume_screener_page():
    ...
    # Removed st.session_state['certificate_html_content'] initialization
    ...
    if jd_text and resume_files:
        ...
        # Only show JD Keyword Cloud for HR/Admin
        if st.session_state.get("user_role") in ["hr", "admin"]:
            st.markdown("---")
        ...
        print(f"Time taken for Individual Resume Analysis: {end_analysis_time - start_time_analysis:.2f} seconds")
                ...
                if selected_candidate_name_for_cert:
                    ...
                    if not candidate_rows.empty:
                        ...
                        if candidate_data_for_cert.get('Certificate Rank') != "Not Applicable":
                            ...
                            # Removed storage in session state: st.session_state['certificate_html_content'] = certificate_html_content 
                            ...
                            if candidate_data_for_cert.get('Email') and candidate_data_for_cert['Email'] != "Not Found":
                                if certificate_pdf_content:
                                    ...
                                    # Add an explicit button for sending email for HR/Admin
                                    if st.button(f"📧 Email Certificate to {candidate_data_for_cert['Candidate Name']}", key="email_cert_hr_button"):
                                ...
                                st.info(f"No email address found for {candidate_data_for_cert['Candidate Name']}. Cannot send automated invitation.")
        ...
        elif st.session_state.get("user_role") == "candidate":
            st.markdown("---")
            st.markdown("## Your Screening Results")
            st.caption("Here are the results for the resume(s) you submitted.")

            if not st.session_state['comprehensive_df'].empty:
                # For candidate, show results for *their* uploaded resumes
                # Assuming the comprehensive_df contains only the resumes they just uploaded
                for idx, candidate_data in st.session_state['comprehensive_df'].iterrows():
                    st.markdown(f"### **Results for: {candidate_data['Candidate Name']}**")
                    st.write(f"**Score:** {candidate_data['Score (%)']:.2f}%")
                    st.write(f"**Years Experience:** {candidate_data['Years Experience']:.1f} years")
                    if pd.notna(candidate_data['CGPA (4.0 Scale)']):
                        st.write(f"**CGPA (4.0 Scale):** {candidate_data['CGPA (4.0 Scale)']:.2f}")
                    st.write(f"**Semantic Similarity:** {candidate_data['Semantic Similarity']:.2f}")
                    
                    st.write(f"**AI Analysis:**")
                    st.markdown(candidate_data['Detailed HR Assessment'])

                    # Certificate for candidate
                    st.markdown("#### Your ScreenerPro Certificate")
                    if candidate_data.get('Certificate Rank') != "Not Applicable":
                        certificate_html_content_single = generate_certificate_html(candidate_data)
                        certificate_pdf_content_single = generate_certificate_pdf(certificate_html_content_single)

                        col_cand_cert_view, col_cand_cert_download = st.columns(2)
                        with col_cand_cert_view:
                            if st.button(f"👁️ View Certificate for {candidate_data['Candidate Name']}", key=f"view_cert_cand_{idx}"):
                                st.components.v1.html(certificate_html_content_single, height=600, scrolling=True)
                        with col_cand_cert_download:
                            if certificate_pdf_content_single:
                                st.download_button(
                                    label=f"⬇️ Download Certificate for {candidate_data['Candidate Name']}",
                                    data=certificate_pdf_content_single,
                                    file_name=f"ScreenerPro_Certificate_{candidate_data['Candidate Name'].replace(' ', '_')}.pdf",
                                    mime="application/pdf",
                                    key=f"download_cert_pdf_cand_{idx}"
                                )
                            else:
                                st.warning("PDF generation failed.")
                        
                        # Email certificate for candidate
                        if candidate_data.get('Email') and candidate_data['Email'] != "Not Found":
                            if st.button(f"📧 Email Certificate to {candidate_data['Candidate Name']}", key=f"email_cert_cand_{idx}"):
                                if certificate_pdf_content_single:
                                    gmail_address = st.secrets.get("GMAIL_ADDRESS")
                                    gmail_app_password = st.secrets.get("GMAIL_APP_PASSWORD")
                                    if send_certificate_email(
                                        ...
                                        certificate_pdf_content=certificate_pdf_content_single,
                                        gmail_address=gmail_address,
                                        gmail_app_password=gmail_app_password
                                    ):
                                        st.success(f"Certificate email sent to {candidate_data['Candidate Name']}!")
                                else:
                                    st.error("Certificate PDF could not be generated for email.")
                        else:
                            st.info(f"No email address found for {candidate_data['Candidate Name']}. Cannot send certificate via email.")
                    else:
                        st.info(f"{candidate_data['Candidate Name']} does not qualify for a ScreenerPro Certificate at this time.")
                    st.markdown("---") # Separator for multiple candidates
            else:
                st.info("No results to display yet. Please upload your resume.")

    else: # If no JD or resume is uploaded
        st.info("Please upload a Job Description and at least one Resume to begin the screening process.")

    # Removed st.session_state['certificate_html_content'] display and deletion at the end of the page
...
if __name__ == "__main__":
    # This block is for when main.py is run directly.
    # The actual application logic for portal selection and page routing
    # is handled in the main part of the script, outside of this __main__ block.
    # The resume_screener_page function is called via the sidebar navigation.
    ...
    # NOTE: This part is duplicated from the main script body. In a real multi-file app,
    # login_section would typically be called once in the main app entry point.
    # For this single-file structure, we keep it here to ensure it runs if main.py is direct entry.
    ...
    # These functions are already defined above, but need to be accessible if __name__ == "__main__"
    # To avoid re-defining, we can just ensure they are in the global scope or imported.
    ...
    if st.session_state.authenticated:
        ...
        if tab == "🏠 Dashboard":
            ...
            resume_screener_page() # Call the resume screener page
