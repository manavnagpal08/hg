...
.top-header {{
    ...
    align-items: center;
    padding: 1rem 2rem;
    background-color: {'#2D2D2D' if dark_mode else 'rgba(255, 255, 255, 0.9)'};
    border-bottom: 1px solid {'#444' if dark_mode else '#E0E0E0'};
    margin-bottom: 2rem;
    border-radius: 15px;
    box-shadow: 0 4px 15px rgba(0,0,0,{'0.2' if dark_mode else '0.05'});
}}
.top-header h1 {{
    margin: 0;
    color: {'#00cec9' if dark_mode else '#00cec9'};
...
.top-header .profile a {{
    color: {'#f0f2f6' if dark_mode else '#007bff'};
    text-decoration: none;
    font-weight: 600;
    transition: color 0.2s ease;
}}
.top-header .profile a:hover {{
    color: {'#00cec9' if dark_mode else '#0056b3'};
}}
</style>
