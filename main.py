...
if not st.session_state.authenticated:
    ...
    with col_hr_portal:
        if st.button("🔹 HR Portal", use_container_width=True, help="Access for HR personnel and Administrators", key="btn_hr_portal"):
            ...
            # Clear other portal's login state if switching
    ...
    if st.session_state.active_portal == "hr_portal":
        ...
        if not authenticated_for_portal:
            st.stop() # Stop if login failed or not yet authenticated for HR portal
        ...
        if not authenticated_for_portal:
            st.stop() # Stop if login failed or not yet authenticated for Candidate portal
    else:
        st.info("Please select a portal to log in or register.")
        st.stop() # Stop execution if no portal is selected and not authenticated
    
# --- Main Application Content (only runs if authenticated) ---
if st.session_state.authenticated:
    # Log successful login (only once per user session)
    ...
    # Initialize comprehensive_df globally for HR portal if it doesn't exist
    ...
    if is_current_user_hr():
        st.header("HR Portal") # Display HR Portal header
        navigation_options = [
        ...
        if is_current_user_admin(): # Only add Admin Tools if the user is an admin
        ...
        tab = st.sidebar.radio("📍 Navigate (HR Portal)", navigation_options, index=navigation_options.index(default_tab), key="hr_portal_sidebar_radio")

    elif is_current_user_candidate():
        st.header("Candidate Portal") # Display Candidate Portal header
        navigation_options = [
        ...
        tab = st.sidebar.radio("📍 Navigate (Candidate Portal)", navigation_options, index=navigation_options.index(default_tab), key="candidate_portal_sidebar_radio")
    else:
        ...
        st.rerun() # Force logout for unknown role
    ...
    if tab == "🏠 Dashboard":
        ...
        st.session_state.pop('user_company', None)
        st.session_state.pop('user_role', None)
        st.session_state.pop('active_portal', None) # Clear active portal on logout
