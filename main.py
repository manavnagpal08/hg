...
# --- Top-level Portal Selection using st.tabs ---
# This ensures only one set of portal content is rendered at a time,
# preventing widget key conflicts.

# Only show portal selection if not authenticated
if not st.session_state.get("authenticated", False):
    ...
    with hr_portal_tab:
        st.header("HR Portal")
        # Call login_section_ui for HR portal. It will handle its own internal tabs (Login/Register).
        authenticated_for_portal = login_section_ui("hr_portal", load_session_data_from_firestore_rest)

        if authenticated_for_portal:
            ...
            # Use a unique key for the sidebar radio within the HR portal
            tab = st.sidebar.radio("📍 Navigate (HR Portal)", navigation_options_hr, index=navigation_options_hr.index(default_tab_hr), key="hr_portal_sidebar_radio")
            ...
            if tab == "🧠 Resume Screener":
                ...
                if is_admin:
                    ...
                    # Use unique keys for admin tabs as well
                    admin_tab = st.tabs(["Create User", "Reset Password", "Toggle User Status", "Activity Log"], key="admin_tools_tabs")
    ...
    with candidate_portal_tab:
        ...
        # Call login_section_ui for Candidate portal. It will handle its own internal tabs (Login/Register).
        ...
        if authenticated_for_portal:
            ...
            # Use a unique key for the sidebar radio within the Candidate portal
            tab = st.sidebar.radio("📍 Navigate (Candidate Portal)", navigation_options_candidate, key="candidate_portal_sidebar_radio")
    ...
    # If authenticated, display the appropriate portal content directly
    if st.session_state.active_portal == "HR Portal":
        # Re-render HR content without portal selection
        st.header("HR Portal")
        # Call login_section_ui for HR portal. It will handle its own internal tabs (Login/Register).
        # This call will return True immediately because the user is authenticated and role matches.
        login_section_ui("hr_portal", load_session_data_from_firestore_rest)

        # If authenticated for HR Portal, proceed with HR-specific content
        # No need for another 'if authenticated_for_portal:' check here as we are in the 'else' block
        # where authentication is already confirmed.
        ...
        # ======================
        # Analytics Dashboard Page Function (defined once)
        # ======================
        def analytics_dashboard_page():
            st.markdown("""
            <style>
        ...
        # Re-render Candidate content without portal selection
        st.header("Candidate Portal")
        # Call login_section_ui for Candidate portal. It will return True immediately.
        login_section_ui("candidate_portal")
