...
from firebase_utils import (
    ...
    FIREBASE_AUTH_SIGNUP_URL, FIREBASE_AUTH_SIGNIN_URL, FIREBASE_AUTH_RESET_PASSWORD_URL,
    FIREBASE_AUTH_LOOKUP_URL, FIREBASE_AUTH_UPDATE_URL,
    get_firestore_document, update_firestore_document, delete_firestore_document,
    fetch_firestore_collection, to_firestore_format, log_activity_to_firestore
)
...
def admin_password_reset_section():
    ...
    user_options = [user['email'] for user in all_users if user['email'] not in ADMIN_USERNAME_EMAILS]
...
def admin_disable_enable_user_section():
    ...
    all_users = get_all_user_profiles_for_admin()
    non_admin_users = [user for user in all_users if user['email'] not in ADMIN_USERNAME_EMAILS]
